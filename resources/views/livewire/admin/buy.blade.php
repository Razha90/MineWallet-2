...
use App\Models\User;
use App\Notifications\TransactionMailNotification;
use App\Notifications\TransactionMailNotificationFailed;
...
new #[Layout('components.layouts.admin')] class extends Component {
    ...
    {
        $this->topups = Transaction::with(['user', 'product'])
    ...
    public function approved($id)
    {
        ...
        if ($topup) {
            ...
            $user->saldo -= $topup->prize;
            $user->save();
 
            $user->notify(new TransactionMailNotification('<h1>Top Up Berhasil Disetujui</h1><p>Top up kamu sebesar Rp ' . number_format($topup->prize, 0, ',', '.') . ' telah disetujui.</p>', $topup->id, $topup->prize));
    ...
    public function rejected($id)
    {
        ...
        if ($topup) {
            ...
            $user = User::find($topup->user_id);
            $user->notify(new TransactionMailNotificationFailed('<h1>Top Up Gagal</h1><p>Top up kamu sebesar Rp ' . number_format($topup->prize, 0, ',', '.') . ' gagal.</p>', $topup->id, $topup->prize));
            ...
            <tbody>
                <template x-for="item in topups" :key="item.id">
                    <tr class="border-b hover:bg-gray-50"
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            Rp <span x-text="formatRupiah(Number(item.prize))"></span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span
