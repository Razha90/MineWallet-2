...
new #[Layout('components.layouts.payment')] class extends Component {
    ...
    {
        try {
            $data = Transaction::with(['user', 'product'])
                ->where('id', $id)
                ->first();
            if ($data->exists()) {
    ...
    {
        try {
            return redirect()->route('payment.gateway')->with('type', 'PULSA')->with('id', $this->datas['id']);
        } catch (\Throwable $th) {
...
<div class="mx-auto mt-[3%] max-w-5xl bg-white p-6 text-gray-800 shadow-xl" x-data="initPaymentTop">
    ...
    <div class="mb-1 text-base">Deskripsi: <span class="font-medium">Top Up Pulsa</span></div>
    <div class="mb-1 text-base">Nominal Transaksi: Rp. <span class="font-medium"
            x-text="formatRupiah(datas.prize)"></span></div>
    <div class="mb-1 text-base">Jumlah Transaksi: x <span class="text-2xl font-medium" x-text="datas.quantity"></span>
    </div>
    ...
    <div class="rounded-xl bg-gray-100 p-4">
        ...
        <div>
            <div class="flex items-center gap-2 text-base">
                ...
                <span>MineWallet</span>
            </div>
            <p class="text-blue-500 text-xl">Rp <span x-text="formatRupiah(datas.prize)"></span> </p>
        </div>
    </div>
