...
new #[Layout('components.layouts.payment')] class extends Component {
    ...
    {
        try {
            return redirect()->route('payment.gateway')->with('type', 'TOPUP')->with('id', $this->datas['id']);
        } catch (\Throwable $th) {
...
<div class="mx-auto mt-[3%] max-w-5xl bg-white p-6 text-gray-800 shadow-xl" x-data="initPaymentTop">
    ...
    <div class="mb-1 text-base">Nama Penguna : <span class="font-medium" x-text="datas.user.name"></span></div>
    <div class="mb-1 text-base">Deskripsi: <span class="font-medium">Top Up</span></div>
    ...
    <div class="mb-1 text-base">Administrasi Transaksi: Rp. <span class="font-medium"
            x-text="formatRupiah(datas.admin)"></span></div>
    <div class="mb-1 text-base">Total Transaksi: Rp. <span class="text-2xl font-medium"
            x-text="formatRupiah(Number(datas.admin)+Number(datas.amount))"></span></div>
    ...
    <div class="rounded-xl bg-gray-100 p-4">
        ...
        <div class="flex items-center gap-2 text-base">
            ...
            <span x-text="datas.bank.name"></span>
        </div>
    </div>

    <!-- Tombol Bagikan -->
    <div class="mt-6">
        <button @click="prosesPayment" class="w-full rounded-lg bg-blue-600 py-2 font-semibold text-white transition hover:bg-blue-500 cursor-pointer">
            lanjutkan
        </button>
    </div>
</div>
