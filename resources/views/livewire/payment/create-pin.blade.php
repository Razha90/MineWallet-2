...
new #[Layout('components.layouts.payment')] class extends Component {
    ...
    {
        $this->condition = session('condition');
    }

    public function createPinUser($pin)
    {
        try {
            $user = auth()->user();
            $user->pin = $pin;
            $user->save();
            $this->dispatch('success', ['message' => 'Berhasil Membuat Pin.']);
            $this->dispatch('ask-question');
        } catch (\Throwable $th) {
            Log::error('Error creating pin: ' . $th->getMessage());
...
<div x-data="initCreatePin" x-init="initPin">
        ...
        <div class="relative max-h-full w-full max-w-md p-4">
            ...
            <div class="relative rounded-lg bg-white shadow-2xl">
                ...
                <div class="flex flex-col items-center gap-y-2 px-4 pb-4">
                    ...
                    <p class="text-center text-lg text-blue-500">Maaf sesi anda telah habis untuk menganti pin, coba
                        lagi!</p>
                    <button @click="back"
    ...
    <div class="mx-auto mt-[3%] max-w-lg select-none" x-init="initCreatePin">
        <h1 class="mb-12 text-center text-4xl font-bold text-purple-600">Buat Pin Anda</h1>
...
<script>
    function initCreatePin() {
        return {
            condition: @entangle('condition').live,
            pinsButton1: [1, 2, 3],
            pinsButton2: [4, 5, 6],
            pinsButton3: [7, 8, 9],
            pins: "",
            ...
            initCreatePin() {
                ...
                this.stopInit = true;
                this.$watch('pins', (value) => {
                    if (value.length >= 6) {
                        this.$wire.createPinUser(value);
                    }
                })
            },
            inputPin(pin) {
